...
export default function BoxAbout({ title, paragraph }) {
  ...
  useEffect(() => {
    ...
    if (imageProfile) {
      setUploadedImageUrl(imageProfile);
      setFormData({
  ...
  const handleSave = async () => {
    ...
    try {
      if (localStorage.getItem("image")) {
        ...
        localStorage.setItem("horoscope", formData.horoscope);
        localStorage.setItem("zodiac", formData.zodiac);
                  ...
                  <div className="image-side mt-7 flex items-center">
                    <label htmlFor="uploadImage" className="image-profile bg-gray-500 mr-5 flex justify-center items-center rounded-lg">
                      {imageSrc ? (
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="white" className="bi bi-plus-lg" viewBox="0 0 16 16">
